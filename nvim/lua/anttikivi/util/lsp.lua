...
function M.format(opts)
  opts = vim.tbl_deep_extend(
    ...
    {},
    opts or {},
    AK.opts("nvim-lspconfig").format or {},
    AK.opts("conform.nvim").format or {}
  )
...
---@param opts? AKFormatter | { filter?: (string | lsp.Client.filter) }
function M.formatter(opts)
  ...
  local ret = {
    ...
    priority = 1,
    format = function(buf)
      M.format(AK.merge({}, filter, { bufnr = buf }))
    end,
    sources = function(buf)
      local clients = M.get_clients(AK.merge({}, filter, { bufnr = buf }))
      ---@param client vim.lsp.Client
